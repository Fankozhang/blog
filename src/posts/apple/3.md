...
<template>
  <div>
        <iframe style="width: 100%; height: 80vh" v-if="url" :src="url" id="myFrame"></iframe>
  </div>
</template>
...
<script>
    ...
    showModal(record) {
      ...
      }/nacga-data-collection/web/#/dataLedger/projectDetails?projNoNacga=${record.projNoNacga}&from=iframe`
      setTimeout(() => {
...
接收：  main.js 接收传来的值，并通过vuex实现存储， 随后调用 store取值。 

```
window.addEventListener('message', (event) => {
  if(event.data.from=='iframe'){
    store.commit('SET_FORM',event.data.from)
  }
})
